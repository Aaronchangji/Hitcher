...
FAISS_PRAGMA_IMPRECISE_FUNCTION_BEGIN
float fvec_inner_product_int8(const int8_t* x, const int8_t* y, size_t d) {
    float res = 0.F;
    FAISS_PRAGMA_IMPRECISE_LOOP
    for (size_t i = 0; i != d; ++i) {
        res += int(x[i]) * int(y[i]);
...
FAISS_PRAGMA_IMPRECISE_FUNCTION_BEGIN
float fvec_L2sqr_int8(const int8_t* x, const int8_t* y, size_t d) {
    size_t i;
    float res = 0;
    FAISS_PRAGMA_IMPRECISE_LOOP
    for (i = 0; i < d; i++) {
        const int tmp = int(x[i]) - int(y[i]);
        res += tmp * tmp;
    }
    return res;
}
