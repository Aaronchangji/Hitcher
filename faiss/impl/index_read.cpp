...
InvertedLists* read_InvertedLists(IOReader* f, int io_flags) {
    ...
    if (h == fourcc("il00")) {
        ...
        ails->ids2.resize(ails->nlist);
        ails->codes2.resize(ails->nlist);
        std::vector<size_t> sizes(ails->nlist);
        read_ArrayInvertedLists_sizes(f, sizes);
        ails->sizes2 = sizes;
        for (size_t i = 0; i < ails->nlist; i++) {
            ails->ids2[i] = new idx_t[sizes[i]];
            ails->codes2[i] = new uint8_t[sizes[i] * ails->code_size];
        }
        for (size_t i = 0; i < ails->nlist; i++) {
            size_t n = sizes[i];
            if (n > 0) {
                READANDCHECK(ails->codes2[i], n * ails->code_size);
                READANDCHECK(ails->ids2[i], n);
            }
        }
        // READ1(ails->nlist);
        // READ1(ails->code_size);
