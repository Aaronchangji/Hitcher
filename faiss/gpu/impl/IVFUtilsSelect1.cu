...
template <
        ...
        int NumThreadQ,
        bool Dir>
__global__ void pass1SelectListsH2(
    ...
    for (IndexT queryId = blockIdx.x; queryId < prefixSumOffsets.getSize(0); queryId += gridDim.x) {
        constexpr auto kInit = Dir ? kFloatMin : kFloatMax;
        BlockSelect<
                float,
                IndexT,
        ...
        // We ensure that before the array (at offset -1), there is a 0 value
        auto offsets = prefixSumOffsets.data();
        auto start = *(&offsets[queryId] - 1);
        auto end = offsets[queryId];
        auto num = end - start;
        auto limit = utils::roundDown(num, (IndexT)kWarpSize);
        ...
        // together
        for (int i = threadIdx.x; i < k; i += blockDim.x) {
            heapDistances[queryId][i] = smemK[i];
            heapIndices[queryId][i] = idx_t(smemV[i]) - start;
        }
    }
}
...
void runPass1SelectLists(
        Tensor<idx_t, 2, true>& prefixSumOffsets, //qid, cid --> offset
        Tensor<float, 1, true>& distance,
        int nprobe,
        int k,
        bool use64BitSelection,
        bool chooseLargest,
        Tensor<float, 3, true>& heapDistances, //qid, sliceid, k --> dis
        Tensor<idx_t, 3, true>& heapIndices, //qid, sliceid, k --> embedid
        cudaStream_t stream) {
    // This is also caught at a higher level
    FAISS_ASSERT(k <= GPU_MAX_SELECTION_K);

    auto grid =
            dim3(heapDistances.getSize(1), //num slice
                 std::min(
                         prefixSumOffsets.getSize(0), //num query
...
void runPass1SelectListsH2(
        // Tensor<idx_t, 1, true>& queryListIds,  //qidx --> cid
        Tensor<idx_t, 1, true>& prefixSumOffsets, //qidx --> offset
        Tensor<float, 1, true>& distance,
        int k,
        bool use64BitSelection,
        bool chooseLargest,
    ...
    auto grid = std::min(prefixSumOffsets.getSize(0), (idx_t)getMaxGridCurrentDevice().x);

#define RUN_PASS(INDEX_T, BLOCK, NUM_WARP_Q, NUM_THREAD_Q, DIR)         \
    do {                                                                \
        pass1SelectListsH2<INDEX_T, BLOCK, NUM_WARP_Q, NUM_THREAD_Q, DIR> \
                <<<grid, BLOCK, 0, stream>>>(                           \
                        prefixSumOffsets,                               \
                        distance,                                       \
                        k,                                              \
