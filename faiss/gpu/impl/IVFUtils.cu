...
__global__ void getQueryListIds(
        idx_t size,
        idx_t *listIds,
        idx_t *queryOffsets,
        idx_t *queryListIds) {
    for (idx_t list_idx = blockIdx.x; list_idx < size; list_idx += gridDim.x) {
        idx_t cur_offset = queryOffsets[list_idx] + threadIdx.x;
        idx_t end_offset = queryOffsets[list_idx + 1];
        idx_t list_id = listIds[list_idx];
        while (cur_offset < end_offset) {
            queryListIds[cur_offset] = list_id;
            cur_offset += blockDim.x;
        }
    }
}

__global__ void getResultLengthsH2(
        Tensor<idx_t, 1, true> ivfListIds,
        idx_t* listLengths,
        idx_t totalSize,
        Tensor<idx_t, 1, true> length) {
    idx_t linearThreadId = idx_t(blockIdx.x) * blockDim.x + threadIdx.x;
    if (linearThreadId >= totalSize) {
        return;
    }
    idx_t centroidId = ivfListIds[linearThreadId];
    // Safety guard in case NaNs in input cause no list ID to be generated
    length[linearThreadId] = (centroidId != -1) ? listLengths[centroidId] : 0;
}
...
void runFillCidsHCC(
    ...
    auto grid = dim3(16);
    auto block = dim3(8);
    getQueryListIds<<<grid, block, 0, stream>>>(
        listIds.getSize(0), listIds.data(), queryOffsets.data(), queryListIds.data()
    );
}

void runCalcListOffsetsHCC(
        GpuResources* res,
        Tensor<idx_t, 1, true>& ivfListIds,
        DeviceVector<idx_t>& listLengths,
    ...
    idx_t totalSize = ivfListIds.getSize(0);
    idx_t numThreads = std::min(totalSize, (idx_t)getMaxThreadsCurrentDevice());
    idx_t numBlocks = utils::divUp(totalSize, numThreads);
    ...
    getResultLengthsH2<<<grid, block, 0, stream>>>(
        ivfListIds,
        listLengths.data(), 
        totalSize, 
        prefixSumOffsets
...
void runCalcQueryOffsetsHCC(
        GpuResources* res,
        Tensor<idx_t, 1, true>& queryOffsets,
        Tensor<char, 1, true>& thrustMem,
        cudaStream_t stream) {

    idx_t totalSize = queryOffsets.numElements();
    // Prefix sum of the indices, so we know where the intermediate
    // results should be maintained
    // Thrust wants a place for its temporary allocations, so provide
    // one, so it won't call cudaMalloc/Free if we size it sufficiently
